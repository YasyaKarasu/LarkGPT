...
func (c *ChatContext) ResetConversation(ID string) {
	session.ClearContextSession(ID)
}

// SaveConversation 保存对话
func (c *ChatContext) SaveConversation(ID string) error {
	...
	return session.SetContextSession(ID, buffer.String())
}

// LoadConversation 加载对话
func (c *ChatContext) LoadConversation(ID string) error {
	dec := gob.NewDecoder(strings.NewReader(session.GetContextSession(ID)))
